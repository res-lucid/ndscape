...
def train_model(split, X, y, model_type, **kwargs):
    ...
    mc1 = data[np.array([bool(int(split[0]) & (1 << int(v))) for v in y], dtype=bool)]
    mc2 = data[np.array([bool(int(split[1]) & (1 << int(v))) for v in y], dtype=bool)]
...
def predict_proba(root, X, c):
    ...
    def predict(node):
        if node.is_leaf():
            assert (proba[:, int(node.split[0]).bit_length() - 1] == 0).all()
            proba[:, int(node.split[0]).bit_length() - 1] = node.p
        else:
