...
def generate_bbok_split(enc):
    rc = np.array([i for i in range(int(enc).bit_length()) if int(enc) & (1 << i)], dtype=int)
    ...
    sub_id = np.random.randint(1, 2 ** (len(rc) - 1))
    ...
    return (
        sum(1 << int(c) for c in c1_group),
        sum(1 << int(c) for c in c2_group),
    )
...
def generate(n, labels=None, seed=42):
    ...
    # rc = sum(1 << int(i) for i in np.unique(y))
    rc = sum(1 << int(i) for i in np.unique(labels))
    np.random.seed(seed)
